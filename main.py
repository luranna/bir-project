...
async def receive_data(request: Request, tempData: SystemData, access_token: str = Cookie(None)):
    if access_token is not None:
...
async def update_system_mode(request: Request, access_token: str = Cookie(None)):
    if access_token is not None:
...
async def get_temp_limits(request: Request, access_token: str = Cookie(None)):
    if access_token is not None:
...
@app.get("/system-mode")
async def get_system_mode(request: Request, access_token: str = Cookie(None)):
    if access_token is not None:
...
async def update_temp_limits(request: Request, minTemp= Form(None), maxTemp=Form(None), access_token: str = Cookie(None)):
    if access_token is not None:
        redirect_url = request.url_for('home')
...
async def update_battery_limits(request: Request, minBattery = Form(None), maxBattery=Form(None), access_token: str = Cookie(None)):
    if access_token is not None:
        redirect_url = request.url_for('home')
